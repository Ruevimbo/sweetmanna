...
function createConfetti(){
  ...
  // Bigger confetti for manna effect
  const size = 8 + Math.random() * 12; // 8px to 20px
  confetti.style.width = size + 'px';
  confetti.style.height = size + 'px';
  confetti.style.background = `#ffd54f`; // golden color
  confetti.style.borderRadius = '50%';
  confetti.style.pointerEvents = 'none';
  confetti.style.opacity = 0.9;
  confetti.style.animation = `fall ${1.5 + Math.random()*2}s linear forwards`; // slightly slower fall
  document.body.appendChild(confetti);

  setTimeout(() => confetti.remove(), 4000); // remove after animation
}

// Update keyframes dynamically if not already
...
setInterval(createConfetti, 300); // spawn every 0.3s for gentle rain
